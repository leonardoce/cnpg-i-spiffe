...
# kind load docker-image --name=${current_context} cnpg-i-spiffe:${VERSION:-latest}
...
load_image_registry cnpg-i-spiffe:${VERSION:-latest}

# Patch the deployment to use the provided image
kubectl patch deployments.apps -n cnpg-system spiffe -p \
  "{\"spec\":{\"template\":{\"spec\":{\"containers\":[{\"name\":\"cnpg-i-spiffe\",\"image\":\"${registry_name}:5000/cnpg-i-spiffe:${VERSION:-latest}\"}]}}}}"
